# Video Game Build Release Engineer repository instructions

This repository contains one standalone C-Sweet protocol-v2 agent. Its purpose is:

> Owns build configuration, adapter operations, packaging, release-readiness evidence, and publication proposals.

## Invariants

- Keep `com.csweet.video-game-build-release-engineer` and version `1.0.0` synchronized between agent code,
...
```powershell
dotnet test
dotnet run --project src/CSweet.Agent.BuildReleaseEngineer.VideoGame -- --self-test
```
